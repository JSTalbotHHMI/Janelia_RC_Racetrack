#!/usr/bin/env python3
"""
Track a red/blue marker pair from a USB webcam and visualize RC-car motion.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Track a red and blue blob pair from a webcam, estimate vehicle pose, "
    ...
    parser.add_argument("--window-name", default="RC Blob Tracker", help="GUI window title. Default: RC Blob Tracker.")
    ...
    parser.add_argument(
        "--car-id",
        type=int,
        default=DEFAULT_CAR_ID,
        help=f"Car identifier embedded in CAR packets. Default: {DEFAULT_CAR_ID}.",
    )
    parser.add_argument(
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.car_id < 0:
        raise SystemExit("--car-id must be >= 0")
...
def choose_blob_pair(red_blobs: list[Blob], blue_blobs: list[Blob], max_pair_distance: float, previous_position: tuple[float, float] | None) -> tuple[Blob, Blob] | None:
    best_pair: tuple[Blob, Blob] | None = None
    ...
    for red_blob in red_blobs:
        for blue_blob in blue_blobs:
            ...
            score = (red_blob.area + blue_blob.area) - (pair_distance * 2.0) - continuity_penalty
            if score > best_score:
                best_score = score
                best_pair = (red_blob, blue_blob)
    return best_pair
...
def draw_pose_overlay(frame: np.ndarray, samples: deque[PoseSample], orientation_full_scale_deg: float, speed_reference_px_s: float, stop_speed_threshold_px_s: float) -> None:
    if not samples:
        return
    ...
    if current.speed_px_s >= stop_speed_threshold_px_s:
...
class TrackerApp:
    def __init__(self, capture: cv2.VideoCapture, args: argparse.Namespace, backend_name: str, actual_width: int, actual_height: int) -> None:
        ...
        self.running = True
        self.samples: deque[PoseSample] = deque(maxlen=args.trail_length)
        ...
        self.motion_reference_timestamp_s: float | None = None
        ...
        self.side_panel.rowconfigure(4, weight=1)
        ...
        self.controls_frame.grid(row=4, column=0, sticky="nsew")

        cv2.namedWindow(self.args.window_name, cv2.WINDOW_AUTOSIZE)
        ...
        if self.serial_state.enabled:
            try:
                ...
                print(f"Starting disconnected because {self.serial_state.port!r} could not be opened: {exc}")
    ...
    def detect_blob_pair(
        self,
        processing_frame: np.ndarray,
        previous_position: tuple[float, float] | None,
        previous_speed_px_s: float,
        dt_s: float | None,
    ) -> tuple[list[Blob], list[Blob], tuple[Blob, Blob] | None]:
        scaled_previous_position = (
            None
            if previous_position is None
            else (
                previous_position[0] * self.processing_scale,
                previous_position[1] * self.processing_scale,
            )
        )
        scaled_previous_speed = previous_speed_px_s * self.processing_scale
        ...
        search_regions: list[tuple[tuple[int, int, int, int] | None, str]] = []
        roi = compute_search_roi(
            processing_frame.shape,
            scaled_previous_position,
            scaled_previous_speed,
            dt_s,
            scaled_max_pair_distance,
        )
        if roi is not None:
            search_regions.append((roi, "ROI"))
            expanded_roi = expand_roi(
                roi,
                processing_frame.shape,
                int(round(ROI_MARGIN_PX * self.processing_scale * 2.0)),
            )
            if expanded_roi is not None and expanded_roi != roi:
                search_regions.append((expanded_roi, "Expanded ROI"))
        if roi is None or (self.consecutive_misses + 1) % FULL_FRAME_RECOVERY_INTERVAL == 0:
            search_regions.append((None, "Full frame"))

        final_red_blobs: list[Blob] = []
        final_blue_blobs: list[Blob] = []
        chosen_pair: tuple[Blob, Blob] | None = None

        for region, mode_name in search_regions:
            if region is None:
                region_frame = processing_frame
                x_offset = 0
                y_offset = 0
            else:
                x0, y0, x1, y1 = region
                region_frame = processing_frame[y0:y1, x0:x1]
                x_offset = x0
                y_offset = y0

            hsv_frame = preprocess_frame(region_frame, self.args.blur_kernel)
            red_blobs = offset_blobs(
                extract_blobs(
            ...
            chosen_pair = choose_blob_pair(
                red_blobs,
                blue_blobs,
                scaled_max_pair_distance,
                scaled_previous_position,
            )
            final_red_blobs = red_blobs
            final_blue_blobs = blue_blobs
            if chosen_pair is not None:
                if region is None:
                    self.last_search_mode = mode_name
                else:
                    self.last_search_mode = f"{mode_name} {region[2] - region[0]}x{region[3] - region[1]}"
                return (
                    scale_blobs(final_red_blobs, self.processing_scale),
                    scale_blobs(final_blue_blobs, self.processing_scale),
                    (
                        scale_blobs([chosen_pair[0]], self.processing_scale)[0],
                        scale_blobs([chosen_pair[1]], self.processing_scale)[0],
                    ),
                )

        self.last_search_mode = search_regions[-1][1] if search_regions else "Full frame"
        return (
            scale_blobs(final_red_blobs, self.processing_scale),
            scale_blobs(final_blue_blobs, self.processing_scale),
            None,
    ...
    def update_frame(self) -> None:
        ...
        dt_s = (
            max(frame_timestamp_s - self.previous_timestamp_s, 1e-6)
            if self.previous_timestamp_s is not None
            else None
        )
        previous_speed_px_s = self.last_pose.speed_px_s if self.last_pose is not None else 0.0
        ...
        red_blobs, blue_blobs, chosen_pair = self.detect_blob_pair(
            processing_frame,
        ...
        if chosen_pair is not None:
            ...
            if self.motion_reference_position is None or self.motion_reference_timestamp_s is None:
                speed_px_s = 0.0
                travel_deg = self.last_pose.travel_deg if self.last_pose is not None else orientation_deg
            else:
                motion_dt_s = max(frame_timestamp_s - self.motion_reference_timestamp_s, 1e-6)
                dx = car_position[0] - self.motion_reference_position[0]
                dy = car_position[1] - self.motion_reference_position[1]
                ...
                if measured_speed_px_s >= self.args.stop_speed_threshold:
                    ...
                    self.motion_reference_position = car_position
                    self.motion_reference_timestamp_s = frame_timestamp_s
                else:
                    speed_px_s = 0.0
                    travel_deg = self.last_pose.travel_deg if self.last_pose is not None else orientation_deg
            ...
            self.previous_timestamp_s = frame_timestamp_s
            if self.motion_reference_position is None or self.motion_reference_timestamp_s is None:
                self.motion_reference_position = car_position
                self.motion_reference_timestamp_s = frame_timestamp_s
            ...
            self.set_status(
                [
                    "Tracking active.",
                    f"Position: ({car_position[0]:.1f}, {car_position[1]:.1f}) px",
                    f"Orientation: {orientation_deg:.1f} deg",
                    f"Travel: {travel_deg:.1f} deg",
                    f"Speed: {speed_px_s:.1f} px/s",
                    f"Framerate: {fps:.1f} fps",
                    f"Relative heading: {wrap_angle_deg(orientation_deg - travel_deg):+.1f} deg",
            ...
            self.set_status(
                [
                    "Waiting for one red blob and one blue blob.",
                    "Make sure both markers are contiguous and close together.",
                    f"Framerate: {fps:.1f} fps",
                    f"Search region: {self.last_search_mode}",
                    f"Detected red blobs: {len(red_blobs)}",
                    f"Detected blue blobs: {len(blue_blobs)}",
        ...
        preview = fit_frame_to_canvas(annotated, PREVIEW_WIDTH_PX, PREVIEW_HEIGHT_PX)
