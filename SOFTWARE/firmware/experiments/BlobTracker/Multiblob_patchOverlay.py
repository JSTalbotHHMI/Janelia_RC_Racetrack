...
DEFAULT_EXTERNAL_CALIBRATION_CSV = Path(
    r"C:\Users\talbotj\Documents\GitHub\Janelia_RC_Racetrack\SOFTWARE\firmware\experiments\tracker\output\Calibrated.csv"
)
DEFAULT_PATCH_CSV = Path(
    r"C:\Users\talbotj\Documents\GitHub\Janelia_RC_Racetrack\SOFTWARE\utilities\patch-generator\output\PatchWithFinish.csv"
)
CALIBRATION_PREFERENCE_FILE = DEFAULT_OUTPUT_DIR / "Multiblob_patchOverlay.default_calibration.txt"
PATCH_PREFERENCE_FILE = DEFAULT_OUTPUT_DIR / "Multiblob_patchOverlay.default_patch.txt"
LEGACY_CALIBRATION_PREFERENCE_FILE = Path(__file__).with_name(
    "Multiblob_patchOverlay.default_calibration.txt"
)
LEGACY_PATCH_PREFERENCE_FILE = Path(__file__).with_name(
    "Multiblob_patchOverlay.default_patch.txt"
)
...
class SplashScreen:
    def __init__(self, image_path: Path) -> None:
        ...
        try:
            ...
            x_pos = max(0, (screen_width - image_width) // 2)
            y_pos = max(0, (screen_height - image_height) // 2)
            self.root.geometry(f"{image_width}x{image_height}+{x_pos}+{y_pos}")
            self.root.update()
        except tk.TclError:
            self.close()
...
def save_default_calibration_path(csv_path: str) -> None:
    CALIBRATION_PREFERENCE_FILE.parent.mkdir(parents=True, exist_ok=True)
    CALIBRATION_PREFERENCE_FILE.write_text(csv_path, encoding="utf-8")


def load_default_calibration_path() -> str | None:
    preference_file = CALIBRATION_PREFERENCE_FILE
    if not preference_file.exists():
        preference_file = LEGACY_CALIBRATION_PREFERENCE_FILE
    if not preference_file.exists():
        return None
    saved_path = preference_file.read_text(encoding="utf-8").strip()
    return saved_path or None


def save_default_patch_path(csv_path: str) -> None:
    PATCH_PREFERENCE_FILE.parent.mkdir(parents=True, exist_ok=True)
    PATCH_PREFERENCE_FILE.write_text(csv_path, encoding="utf-8")


def load_default_patch_path() -> str | None:
    preference_file = PATCH_PREFERENCE_FILE
    if not preference_file.exists():
        preference_file = LEGACY_PATCH_PREFERENCE_FILE
    if not preference_file.exists():
        return None
    saved_path = preference_file.read_text(encoding="utf-8").strip()
    return saved_path or None
...
def load_patch_overlay(csv_path: str) -> list[PatchOverlayRecord]:
    ...
    if not patches:
        raise ValueError(f"No patch rows found in {patch_path}.")
    return patches
...
class TrackerApp:
    def __init__(self, capture: cv2.VideoCapture, args: argparse.Namespace, backend_name: str, actual_width: int, actual_height: int) -> None:
        ...
        self.side_panel.rowconfigure(7, weight=1)
        ...
        self.status_text.grid(row=5, column=0, sticky="nsew", pady=(12, 12))
        self.status_scrollbar.grid(row=5, column=1, sticky="ns", pady=(12, 12))
        self.status_text.configure(state="disabled")

        self.controls_frame = ttk.Frame(self.side_panel)
        self.controls_frame.grid(row=6, column=0, sticky="ew", pady=(0, 12))
        ...
        self.spacer_frame.grid(row=7, column=0, sticky="nsew")
